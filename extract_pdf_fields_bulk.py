#!/usr/bin/env python3
"""
Extract form fields from PDF templates and create text files with field information.
"""
...
def main():
    """
    Process all PDF files in the TEMPLATES folder and create corresponding text files
    ...
    for pdf_file in pdf_files:
        ...
        # Create output filename by replacing .pdf with _fields.txt
        ...
        if os.path.exists(pdf_path):
            ...
            if field_count > 0:
                print(f"  ✓ Extracted {field_count} fields -> {output_filename}")
                success_count += 1
                total_fields += field_count
            else:
                print(f"  ⚠ No fields found or error occurred")
        else:
    ...
    print(f"  - Output files created in: {outputs_dir}")
